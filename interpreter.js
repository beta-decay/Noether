var code, stack, position; var output, variables, funcpos; var temppos, exitcondition, delaypos; var ifstage, testval, test; var functions; var funccode;

String.prototype.shuffle = function () { var a = this.split(""), n = a.length;
...
function check() { currChar = code[position]; if (currChar == "P") { output += stack[stack.length-1]; position += 1 } else if (".0123456789N".indexOf(currChar) !== -1) { try { n = ""; while ("0123456789.N".indexOf(code[position]) !== -1) { if (code[position] == "N") { n += "-"; } else { n += code[position]; } position += 1; } stack.push(Number(n)); } catch(e) { stack.push(Number(n)); } } else if (currChar == " ") { position += 1; } else if (currChar == "+") { A = stack.pop(); B = stack.pop(); stack.push(B+A); position += 1; } else if (currChar == "-") { A = stack.pop(); B = stack.pop();
...
} else if (currChar == "M") { A = stack.pop(); B = stack.pop();
...
function runCode() { while(position < code.length){ position = check(); } }

function interpreter() { code = document.getElementById("code").value; position = 0; stack = []; variables = {}; exitcondition = []; temppos = []; output = "";
